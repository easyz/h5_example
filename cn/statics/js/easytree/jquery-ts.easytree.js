    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        this.init = function (jQueryContext, options) {
            ...
            if (_settings.dataUrl) {
                ...
                json = convertInputDataToJson($this.text());         
                if (!json) {
                    alert("EasyTree: Invalid data!");
                    return this;
                }
        ...
        this.toggleNode = function (id) {
            ...
            toggleNodeBegin(null, _nodes, node);
        };
        this.toggleNodes = function (item) {
            if (!item) { return; }
            
            toggleNodeBegin(null, _nodes, item);
        };
        ...
        function nodeClick(event) {
            ...
            if (_settings.stateChanged) { // fire stateChanged event
                ...
                _settings.stateChanged(nodes, j,node);
            }
        }
        ...
        function buildTree(nodes) {
            ...
            for( var i=0; i < nodes.length; i++ ) {
            	if( nodes[i].inmenu === false) {
            		console.log(nodes.splice(i,1));
            	} else if ( nodes[i].children ) {
            		for(var j = 0 ; j < nodes[i].children.length; j++) {
            			if(nodes[i].children[j].inmenu === false){
            				console.log(nodes[i].children.splice(j,1));
            			}
            		}
            	}
            }
